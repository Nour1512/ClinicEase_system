...
// const gridEl = document.getElementById("patientGrid");
// const searchInput = document.getElementById("searchInput");
// const genderFilter = document.getElementById("genderFilter");
// const patientCountPill = document.getElementById("patientCountPill");
// const patientCountSubtitle = document.getElementById("patientCountSubtitle");
// const filterSummary = document.getElementById("filterSummary");
...
//         gridEl.innerHTML = `<div style="text-align: center; padding: 2rem; color: #dc2626;">
//             <p><strong>Error loading patients</strong></p>
//         </div>`;
//     }
// }
...
//         card.innerHTML = `
//             <div class="badge-id">${pat.patient_id}</div>
//             <div class="medicine-card-header">
//                 <div class="medicine-avatar">
//                     ${pat.full_name.split(' ')[0][0]}${pat.full_name.split(' ')[1] ? pat.full_name.split(' ')[1][0] : ''}
//                 </div>
//                 <div>
//                     <div class="medicine-title">${pat.full_name}</div>
//                     <div class="medicine-subtitle">Age: ${age} • ${pat.gender}</div>
//                 </div>
//             </div>
//             <div class="medicine-footer">
...
//                 </div>
//                 <div class="card-actions">
//                     <button class="btn-mini buy" data-action="view" data-id="${pat.patient_id}">View</button>
//                 </div>
//             </div>
//             <div class="medicine-meta" style="margin-top:8px; padding-top:8px; border-top:1px solid rgba(229, 231, 235, 0.6);">
//                 <span>Address: <strong>${pat.address}</strong></span>
//                 <span style="padding: 2px 8px; border-radius: 999px; background: #dcfce7; color: #15803d; font-size: 11px;">Emergency: ${pat.emergency_contact}</span>
//             </div>
//         `;
//         gridEl.appendChild(card);
//     });
    
//     patientCountPill.textContent = `${filtered.length} patient${filtered.length !== 1 ? "s" : ""}`;
...
//     const q = searchInput.value.trim();
//     if (!gender && !q) return "Showing all patients";
//     if (q) parts.push(`matching "${q}"`);
//     if (gender) parts.push(`gender ${gender}`);
//     return `Showing ${currentCount} patient${currentCount !== 1 ? "s" : ""} ${parts.join(", ")}`;
// }
...
function renderGrid() {
    ...
    filtered.forEach(pat => {
        ...
        const names = pat.full_name.split(' ');
        ...
        card.innerHTML = `
            <div class="badge-id">${pat.patient_id}</div>
            <div class="medicine-card-header">
                <div class="medicine-avatar">${firstInitial}${lastInitial}</div>
                <div>
                    <div class="medicine-title">${pat.full_name}</div>
                    <div class="medicine-subtitle">Age: ${age} • ${pat.gender}</div>
                </div>
...
function init() {
    ...
    newPatientModal?.addEventListener("click", (e) => {
        if (e.target === newPatientModal) closeModal();
    });
    
    // Submit form
    newPatientForm?.addEventListener("submit", submitNewPatient);
    
    // Close modal on Escape key
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && !newPatientModal.classList.contains("hidden")) {
            closeModal();
        }
    });
}
